...
from breezyslam.sensors import LidarModel
...
class CoreSLAM(abc.ABC):
    ...
    def __init__(self, 
        laser: LidarModel, map_size_pixels: int, map_size_meters: float, 
        map_quality: int=_DEFAULT_MAP_QUALITY, hole_width_mm: 
        float=_DEFAULT_HOLE_WIDTH_MM):
        '''
    ...
    def update(self, 
            scans_mm: Iterable[float], 
            pose_change: Tuple[float, float, float], 
            scan_angles_degrees: Optional[Iterable[float]]=None, 
            should_update_map: bool=True) -> None:
        '''
    ...
    def getmap(self, mapbytes: bytearray):
        '''
    ...
    def setmap(self, mapbytes: bytearray):
        '''
    ...
    @abc.abstractmethod
    def _updateMapAndPointcloud(self, dxy_mm: float, dtheta_degrees: float, should_update_map: bool) -> None: ...
    ...
    def _scan_update(self, 
        scan: pybreezyslam.Scan, 
        scans_distances_mm: Iterable[float], 
...
class SinglePositionSLAM(CoreSLAM):
    ...
    def _updateMapAndPointcloud(self, dxy_mm: float, dtheta_degrees: float, should_update_map: bool):
        '''
    ...
    @abc.abstractmethod
    def _getNewPosition(self, start_position: pybreezyslam.Position) -> pybreezyslam.Position: ...

    def _costheta(self) -> float:
        return math.cos(self._thetaradians())
 
    def _sintheta(self) -> float:
        return math.sin(self._thetaradians())
        
    def _thetaradians(self) -> float:
...
class RMHC_SLAM(SinglePositionSLAM):
    ...
    def __init__(self, 
        ...
        map_quality: int=_DEFAULT_MAP_QUALITY, 
        hole_width_mm: float=_DEFAULT_HOLE_WIDTH_MM,
        random_seed: Optional[int]=None, 
        sigma_xy_mm: float=_DEFAULT_SIGMA_XY_MM, 
        sigma_theta_degrees: float=_DEFAULT_SIGMA_THETA_DEGREES, 
        max_search_iter: int=_DEFAULT_MAX_SEARCH_ITER):
        '''
        ...
        super().__init__(laser, map_size_pixels, map_size_meters, map_quality, hole_width_mm)
    ...
    def update(self, scans_mm: Iterable[float], pose_change=None, scan_angles_degrees=None, should_update_map=True):
        if not pose_change:
            pose_change = (0, 0, 0)
    
        super().update(scans_mm, pose_change, scan_angles_degrees, should_update_map)   
